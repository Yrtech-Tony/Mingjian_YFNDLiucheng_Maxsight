...
namespace XHX.View
{
    public partial class AnswerSubjectForm : Form
    {
        ...
        public void Transfor(string projectCode, string subjectCode, string shopCode, int order, char checktype)
        {
            ...
            {
                if (_UserInfoDto.RoleType == "I" && _pageName == "ExecuteTeamAlter")
                {
                    DataSet ds1 = service.SearchRecheckStatus(ProjectCode_Golbal, ShopCode_Golbal);
                    string statusCode = "";
...
/// <returns></returns>
        public bool GetSubject(DataSet ds, ButtonType buttonType)
        {
            ...
            if (string.IsNullOrEmpty(subjectDto.PhoteScore.ToString()))
            {
        ...
        private void AnswerSubjectForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            decimal fullScore = 0;
            ...
            //保存问卷信息
            service.SaveAnswer(ProjectCode_Golbal, SubjectCode_Golbal, ShopCode_Golbal,
                            score, txtRemark.Text, "", _UserInfoDto.UserID,
                            CheckType, passReCheck, DateTime.Now.ToString("yyyy-MM-dd"), DateTime.Now.ToString("yyyy-MM-dd"), txtPhotoScore.Text);
